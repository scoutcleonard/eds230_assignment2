...
results_means <- lapply(as.data.frame(results), mean) 

results_df <- cbind(price, average_yield, results_means) %>% 
  as.data.frame()
```
...
```{r}
#plot results
plot_1 = ggplot(results_df, aes(x = price, 
                                y = results_means,
                                fill = results_means)) +
                  geom_point()

plot_2 = ggplot(results_df, aes(x = average_yield, 
                                y = results_means)) +
                  geom_point(color = results_means) 
